var Joi, Promise, createTable, getSong, getSongId, getSongs, nullEmptyStrings, prefixTableName, putSong, removeSong, songSchema, songsTableName, _ref;
...
getSongId = function(serviceId, serviceSongId) {
  var encodedSericeSongId;
  encodedSericeSongId = new Buffer(serviceSongId).toString('base64');
  return "" + serviceId + "." + encodedSericeSongId;
};
...
removeSong = function(app, userId, songId) {
  return app.dbDoc().deleteItemAsync({
    TableName: app.config.DYNAMODB_TABLE_SONGS,
    Key: {
      userId: userId,
      songId: songId
